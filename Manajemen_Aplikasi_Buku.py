...
class AplikasiManajemenBuku:
    def __init__(self, root):
        ...
        tk.Button(frame_tombol, text="Tambah Buku",
                  command=self.tambah_buku).pack(side=tk.LEFT, padx=5)
        tk.Button(frame_tombol, text="Hapus Buku",
                  command=self.hapus_buku).pack(side=tk.LEFT, padx=5)
        tk.Button(frame_tombol, text="Cari Buku",
                  command=self.cari_buku).pack(side=tk.LEFT, padx=5)
    ...
    def cari_buku(self):
        pencarian = tk.simpledialog.askstring(
            "Cari Buku",
        ...
        if pencarian:
            ...
            for buku in self.daftar_buku:
                if (pencarian.lower() in buku.judul.lower()
                        or pencarian.lower() in buku.penulis.lower()):
                    hasil.append(buku)
